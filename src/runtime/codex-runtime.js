...
function buildPrompt({ market, evidence, settings, riskDocPath, marketPath, evidencePath, upstreamContext = null }) {
  ...
  if (localeIsChinese) {
    return [
      ...
      "5. confidence 分级标准（重要：confidence 反映的是你相对市场的认知增量，而非绝对证据量）：",
      "   - high：有多条独立、新鲜（7天内）、高可信度证据，且推理链清晰指向一个方向，市场可研究，且你的判断基于市场参与者不易获取或未充分消化的信息。",
      "   - medium（常见选择）：满足以下条件时给 medium：",
      "     * 有至少1条14天内的相关证据",
      "     * 市场类型有公开可查的数据源（天气预报、赛程表、官方日历、民调、链上数据）",
      "     * 你能基于公开信息形成有方向性的概率判断（即使不完全确定）",
      "     * 且你认为自己的分析相对盘口存在一定信息优势（即你发现了市场可能未充分反映的信息或逻辑）",
      "     政治选举有民调/公开声明、宏观经济有官方数据预期、体育有赛程/伤病/历史对阵、天气有预报模型、科技有公司公告/产品路线图、加密有链上数据/监管文件时，均应给 medium。",
      "   - low（降级条件 - 满足任一即应给 low）：",
      "     * 证据为零且无公开数据源可查",
      "     * 结算规则完全不清、纯粹依赖内幕信息、或市场不可交易",
      "     * 虽有公开证据或数据源，但你判定相对盘口无独立信息优势（证据已被市场充分定价，市场参与者已知道你知道的一切）",
      "     * 你的推理本质上只是锚定盘口价格做微调，而非基于独立分析得出不同结论",
      "   关键原则：有证据 ≠ 有信息优势。如果你在 reasoning_summary 中写到'缺乏独立信息优势'、'证据存在关键缺口'或'无法找到市场未反映的信息'，confidence 必须为 low。",
      "6. 估算方法（两阶段法）：",
      "   阶段一：独立估算",
  ...
  return [
    ...
    "   - medium: at least 1 relevant evidence item within 14 days forming a directional inference, AND you believe your analysis has some information advantage over the market (i.e., you identified information or logic not fully reflected in market price). Political markets with polls/public statements, macro-economics with official data expectations, sports with schedules/injuries/historical matchups, tech with company announcements/roadmaps, crypto with on-chain data/regulatory filings — all qualify for medium when they provide edge over market consensus.",
    "   - low (downgrade conditions - any one triggers low):",
    "     * Zero evidence and no public data sources available",
    "     * Unclear settlement rules, completely unresearchable, purely dependent on insider information, or market is not tradable",
    "     * Evidence exists but you judge there is no independent information advantage over market (evidence already fully priced in, market participants already know everything you know)",
    "     * Your reasoning is essentially anchoring to market price with minor adjustment, rather than reaching a different conclusion from independent analysis",
    "   Key principle: having evidence ≠ having information advantage. If your reasoning_summary states 'no independent information advantage', 'key evidence gaps', or 'cannot find information not reflected by market', confidence MUST be low.",
...
function buildEventGroupPrompt({ markets, evidenceMap, settings, riskDocPath, upstreamContexts = null }) {
  ...
  if (localeIsChinese) {
    return [
      ...
      "4. 必须区分盘口价格和独立证据；盘口价格只能作为对照基准。必须对每个候选人判断是否可研究、证据是否足够独立新鲜、是否存在相对盘口的信息优势；把判断写进对应候选人的 reasoning_summary。",
      "5. 对每个候选人分别判断 confidence（confidence 反映相对市场的认知增量，而非绝对证据量）：",
      "   - high：有多条独立、新鲜（7天内）、高可信度证据，推理链清晰，且你的判断基于市场参与者不易获取或未充分消化的信息。",
      "   - medium：满足以下条件时给 medium：",
      "     * 有至少1条14天内的相关证据",
      "     * 或有公开可查的数据源（民调、赛程表、官方日历、链上数据等）",
      "     * 你能基于公开信息形成有方向性的概率判断",
      "     * 且你认为对该候选人的分析相对盘口存在一定信息优势（即你发现了市场可能未充分反映的信息或逻辑）",
      "   - low（降级条件 - 满足任一即应给 low）：",
      "     * 证据为零且无公开数据源可查",
      "     * 结算规则完全不清、纯粹依赖内幕信息、或市场不可交易",
      "     * 虽有公开证据或数据源，但你判定相对盘口无独立信息优势（证据已被市场充分定价，市场参与者已知道你知道的一切）",
      "     * 你的推理本质上只是锚定盘口价格做微调，而非基于独立分析得出不同结论",
      "   关键原则：有证据 ≠ 有信息优势。如果推理中提到'缺乏独立信息优势'或'证据存在关键缺口'，confidence 必须为 low。",
      "6. 估算方法（两阶段法，对每个候选人）：",
      "   阶段一：独立估算",
      ...
      "   阶段二：与盘口对比审视",
      "   - 将联合分布与盘口价格对比。",
      "   - 对任何偏离盘口超过15pp的候选人，必须在 deviation_justification 中明确说明支撑偏离的具体证据。",
      "   - 如果偏离盘口超过25pp且无法给出有力的 deviation_justification，应重新审视推理过程。",
      "   - 如果对某候选人完全没有认知且证据为零，在该候选人的 uncertainty_factors 中注明 'no_specific_knowledge'。",
      ...
      "   - 在每个受影响候选人的 uncertainty_factors 中注明 'market_price_anchored'；",
  ...
  return [
    ...
    "4. Separate market prices from independent evidence; market prices are a comparison baseline, not supporting evidence. For each outcome, assess whether it is researchable, whether evidence is sufficiently independent and fresh, and whether there is an information advantage over market price; include that assessment in the outcome's reasoning_summary.",
    "5. Per-outcome confidence levels (confidence reflects your informational edge over the market, not absolute evidence quantity):",
    "   - high: multiple independent, fresh (within 7 days), high-credibility evidence with clear reasoning chain, and your judgment is based on information not easily accessible to or not fully digested by market participants.",
    "   - medium: at least 1 relevant evidence item within 14 days, or publicly available data sources exist, AND you believe your analysis has some information advantage over market price for this outcome.",
    "     Requirements: you can form a directional probability judgment based on public info, AND you believe you identified information or logic not fully reflected in market price.",
    "   - low (downgrade conditions - any one triggers low):",
    "     * Zero evidence and no public data sources available",
    "     * Unclear settlement rules, completely unresearchable, purely dependent on insider information, or market is not tradable",
    "     * Evidence exists but you judge there is no independent information advantage over market (evidence already fully priced in, market participants already know everything you know)",
    "     * Your reasoning is essentially anchoring to market price with minor adjustment, rather than reaching a different conclusion from independent analysis",
    "   Key principle: having evidence ≠ having information advantage. If your reasoning states 'no independent information advantage' or 'key evidence gaps', confidence MUST be low.",
    ...
    "   - If you have zero knowledge about a specific outcome and zero evidence, include 'no_specific_knowledge' in that outcome's uncertainty_factors.",
    ...
    "   - Include 'market_price_anchored' in each affected outcome's uncertainty_factors;",
