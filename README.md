...
- 概率校准层（ProbabilityCalibrationLayer）在 AI 概率估算之后、决策引擎之前运行，根据 confidence、researchability、information_advantage、evidence freshness、evidence count、liquidity、days-to-resolution、pre-screen 分类和 unjustified market deviation 对原始概率做 shrinkage 校准（向 0.5 先验收缩），输出 rawProbability、calibratedProbability 和 calibrationReasons。其中 unjustified deviation 维度在 AI 概率偏离盘口>15pp 且未提供 `deviation_justification` 时触发收缩（15-25pp 收缩 0.15，>25pp 收缩 0.35），有充分偏离解释时不额外收缩。**DecisionEngine 使用 calibratedProbability 作为交易决策的概率输入**，防止 LLM 概率下限倾向（如无法区分 0.2% 和 5%）导致在极端低概率市场上错误开仓。校准后概率 clamp 范围为 [0.01, 0.99]。
...
- 候选排序增加了 Downside Risk Ranking（DownsideRiskRanker）：在原有 monthly return / net edge / Kelly 排序基础上，计算每个机会的下行风险评分（概率加权最大亏损、流动性风险、时间风险、价差风险、edge 质量）和跨轮资金分配惩罚（类别集中度、事件重复、可用资本比例、边际递减），输出 risk-adjusted score 用于最终排序。排序额外引入类别优先级乘数（`CATEGORY_PRIORITY_MAP`：politics/geopolitics 1.3×、economics 1.2×、sports 0.7×、mentions 0.6×），短期体育赛事（<3天到期）再施加 0.5× 惩罚，使政治/政策等长期可研究市场优先消耗 daily limit 预算。这对齐了 Predict-Raven 的 downside risk ranking 和 cross-round capital allocation。
- Uninformed Prior 检测（`uninformed-prior-detector.js`）：当 AI 概率估算接近 0.5（±5pp）且推理链表明无实质证据时（通过 `uncertainty_factors` 含 `"no_specific_knowledge"`/`"insufficient_evidence"`、`reasoning_summary` 含无信息关键词、或 `freshness_score < 0.2` 识别），标记为 uninformed prior 并在 DecisionEngine 中以 `"uninformed_prior"` 原因阻断开仓。概率校准层额外对 uninformed prior 施加 0.5 收缩力度，收缩目标为盘口价格（非 0.5 先验），防止 AI 在无法获取证据时产生虚假 edge。配置：`PULSE_UNINFORMED_PRIOR_THRESHOLD`（默认 0.05）。
...
- [x] 多选项市场（Multi-Outcome）联合建模改进：
  ...
  **改进路线图**：
  ...
  - [x] Phase 2 — 联合概率估计 Prompt（Prompt + 代码）｜**对应 Step: Prediction**：`ProbabilityEstimator` 新增批量入口 `estimateEventGroup(markets[])`，同 `eventGroup` 的子市场合并为一次 AI 调用；prompt 从"评估这个市场的 Yes 概率"改为"以下是同一事件的 N 个候选人及各自盘口价格，请给出联合概率分布"；AI 能看到完整竞争格局，估计更准确。
  - [x] Phase 3 — 归一化概率分布 Schema（Prompt + Schema + 代码）｜**对应 Step: Prediction（输出层）**：输出 schema 从 `{ai_probability: number}` 扩展为 `{outcomes: [{label, market_slug, probability}], distribution_confidence}`，prompt 强制"概率之和必须等于 1"约束；代码层后处理做归一化修正（按比例缩放至 sum=1），并校验偏离度（AI 原始 sum 与 1 的偏差 > 20% 时记录 warning）。
  - [x] Phase 4 — 组内最优选择（纯代码，~40-60 行）｜**对应 Step: Decision（DecisionEngine.analyze）**：`DecisionEngine` 新增 `analyzeEventGroup(candidates[], jointProbabilities[])` 方法，基于联合概率向量一次性计算所有子市场 Yes/No 双方向的 edge/Kelly/monthlyReturn，返回组内全局最优的 1-2 个交易（如同时做多最被低估的候选人 + 做空最被高估的候选人）。
  - [x] Phase 5 — 放松事件锁定 + 组合约束（纯代码，~20 行）｜**对应 Step: Risk（RiskEngine.evaluate）+ Candidate Building（dedupeKeys）**：去掉"交易一个即锁死整个事件"的硬去重，替换为 `RiskEngine` 中的事件级组合约束——同一事件最大总敞口 ≤ `maxEventExposurePct`；同一事件内互斥方向（如同时买 A-Yes 和 B-Yes，若 A、B 互斥）的总成本不超过 1 份本金；允许在风控约束内同时持有多个同事件头寸。
  - [x] Phase 6 — 跨子市场约束传播（纯代码，~15 行）｜**对应 Step: Prediction → Decision 之间（Calibration 层）**：当 Phase 2 联合估计不可用时（如部分子市场单独到达），用已有高信度兄弟估计约束当前市场 edge 范围——若兄弟 A 已估 55%（高信度），则当前市场的 AI 概率上限为 45%（减去其他已知兄弟概率），避免逻辑矛盾（"A 有 10% edge，B 也有 10% edge，但 A+B 概率已超 100%"）。
  - [x] Phase 7 — 多臂 Kelly 组合优化（纯代码，~30 行）｜**对应 Step: Decision（Kelly sizing）**：对同一事件内多个入选交易，用互斥结果 Kelly 公式联合优化仓位分配，替代逐个独立 quarter-Kelly 后再被 `maxEventExposurePct` 粗暴截断。

### 已知问题

#### 结构化套利检测存在漏洞

`detectStructuralArbitrage` 通过检查同一 event 下所有子市场的 `sumYesAsk` 是否小于 1 来发现套利机会。当前实现存在以下漏洞：

- **不完整子市场覆盖**：套利检测只基于当前扫描轮次获取到的子市场，不验证是否覆盖了该事件的全部互斥结果。如果一个事件有 10 个候选人但只扫描到 2 个（因为其他子市场流动性不够或不在当前页），`sumYesAsk` 会异常低（如 0.005），系统会误判为"保证盈利 99.5%"的套利机会，实际上只是缺失了其他子市场的概率。
- **影响**：会在极低价格子市场（如 $0.002-$0.003）开仓，这些市场的结算概率远高于所显示的，导致几乎确定亏损。
- **临时规避**：可提高 `ARBITRAGE_MIN_PROFIT` 阈值或设置 `ARBITRAGE_ENABLED=false` 禁用。
- **根本修复方向**：需要在执行套利前验证扫描到的子市场数量是否等于事件声明的结果总数，或者当 `sumYesAsk` 异常低（如 < 0.5）时拒绝执行。
